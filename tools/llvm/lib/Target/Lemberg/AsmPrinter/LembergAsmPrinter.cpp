...
namespace {
  class LembergAsmPrinter : public AsmPrinter {
  public:
...
isBlockOnlyReachableByFallthrough(const MachineBasicBlock *MBB) const {
  ...
  // If the predecessor is an indirect branch, assume a jump table
  // implementation, so it is not a fall through.
  if (Pred && !Pred->empty()) {
	  MachineBasicBlock::const_reverse_iterator RI = Pred->rbegin(), RE = Pred->rend();
	  // Look past delay slot
	  while (RI != RE &&
			 (RI->getOpcode() == Lemberg::NOP
			  || RI->getOpcode() == Lemberg::SEP)) {
		  RI++;
	  }
	  if (RI->getDesc().isIndirectBranch()) {
		  return false;
	  }
  }
